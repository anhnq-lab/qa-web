...
import { Send, Clock, ShieldCheck, Star, MapPin, Phone, Mail, ChevronDown } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
              ...
              <form className="flex flex-col gap-5" onSubmit={(e) => e.preventDefault()}>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <label className="flex flex-col gap-1.5">
                    <span className="text-sm font-semibold text-slate-300">Họ và tên</span>
                    <input type="text" placeholder="Nguyễn Văn A" className="w-full rounded-lg border-navy-700 bg-navy-950 text-white focus:border-primary focus:ring-primary px-4 py-3 text-sm placeholder:text-slate-500" />
                  </label>
                  <label className="flex flex-col gap-1.5">
                    <span className="text-sm font-semibold text-slate-300">Số điện thoại</span>
                    <input type="tel" placeholder="0912 xxx xxx" className="w-full rounded-lg border-navy-700 bg-navy-950 text-white focus:border-primary focus:ring-primary px-4 py-3 text-sm placeholder:text-slate-500" />
                  </label>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <label className="flex flex-col gap-1.5">
                    <span className="text-sm font-semibold text-slate-300">Email</span>
                    <input type="email" placeholder="email@congty.com" className="w-full rounded-lg border-navy-700 bg-navy-950 text-white focus:border-primary focus:ring-primary px-4 py-3 text-sm placeholder:text-slate-500" />
                  </label>
                  <label className="flex flex-col gap-1.5">
                    <span className="text-sm font-semibold text-slate-300">Tên công ty</span>
                    <input type="text" placeholder="Công ty Xây dựng ABC" className="w-full rounded-lg border-navy-700 bg-navy-950 text-white focus:border-primary focus:ring-primary px-4 py-3 text-sm placeholder:text-slate-500" />
                  </label>
                </div>
                <label className="flex flex-col gap-1.5">
                  <span className="text-sm font-semibold text-slate-300">Nội dung cần tư vấn</span>
                  <textarea rows={3} placeholder="Mô tả ngắn gọn nhu cầu..." className="w-full rounded-lg border-navy-700 bg-navy-950 text-white focus:border-primary focus:ring-primary px-4 py-3 text-sm placeholder:text-slate-500 resize-none"></textarea>
                </label>
                
                <button type="submit" className="mt-2 w-full bg-secondary hover:bg-secondary-hover text-navy-950 font-bold rounded-lg py-3.5 px-4 shadow-lg shadow-amber-500/20 transition-all active:scale-[0.98] text-base flex items-center justify-center gap-2">
          ...
          <div className="flex flex-col h-full order-1 lg:order-2">
             <div className="relative w-full aspect-[4/3] lg:aspect-auto lg:h-full min-h-[400px] rounded-2xl overflow-hidden shadow-2xl group border border-navy-800">
               <div className="absolute inset-0 bg-cover bg-center transition-transform duration-700 group-hover:scale-105" style={{ backgroundImage: 'url(https://picsum.photos/800/1000?random=50)' }}></div>
